...
const GUESS_OPERATION = `// возвращает 1, если n слишком большое число, -1, если слишком маленькое, 0, если верное
...
const BINARY_SEARCH_RANGE = `function binarySearch(low, high) {
  while (low <= high) {
  ...
  return (
    <>
      ...
      <p className="mb-2">
        ...
        маленькое, либо слишком большое.
      </p>
      <p className="mb-2">
        Если ваша догадка слишком мала, вам придется угадать число среди тех, которые
        больше вашей догадки, а если слишком велика, то вам придется угадать число среди
        меньших чисел. Различие от поиска состоит в том, что вы не знаете, о каком числе
        думает ваш друг (значение <code>target</code>). Всё, что у вас есть — это обратная
        связь от друга каждый раз, когда вы угадываете, и вы соответствующим образом
        корректируете следующее предположение.
      </p>
      <p className="mb-2">
        Как мы будем устанавливать наши границы? Мы можем применить стандартный шаблон
        бинарного поиска, который мы изучили ранее, за исключением того, что вместо того,
        чтобы проверять, соответствует ли ваш <code>mid</code> значению{' '}
        <code>target</code>, вы должны спросить своего друга, верна ли ваша догадка.
      </p>
      <p className="mb-2">
        Конечно, у вашего друга есть операция определения того, слишком ли ваша догадка
        велика или мала. Всё, что он делает, это сравнивает ваше предположение с числом, о
        котором он думает, и сообщает вам, следует ли вам угадать большее или меньшее
        число. Предположим, что ваш друг загадал число 10. Псевдокод этой операции будет
        выглядить следующим образом:
      </p>
      <Code value={GUESS_OPERATION} />
      <p className="mb-2">
        Теперь всё, что нам нужно сделать, это вызвать эту функцию в нашей функции
        бинарного поиска.
      </p>
      <Code value={BINARY_SEARCH_RANGE} />
      <p className="mb-2">
        Как уже отмечалось, на основе значения, возвращаемого из функции{' '}
        <code>isCorrect</code>, мы можем перемещать наши <code>low</code> и{' '}
        <code>high</code>, которые, как говорилось ранее, являются просто ещё одним
        способом представления указателей <code>L</code> и <code>R</code>.
      </p>
      <p className="mb-2">
        Поиск в диапазоне может сбивать с толку и может показаться очень тонким, потому
        что вам нужно придумать, как изменить типичную реализацию бинарного поиска. Для
        подобных задач предоставляется предопределённый метод API, в данном случае{' '}
        <code>isCorrect</code>, и вы должны рассматривать функцию как чёрный ящик и
        использовать её в своём собственном методе бинарного поиска.
      </p>
      <p className="mb-2">
